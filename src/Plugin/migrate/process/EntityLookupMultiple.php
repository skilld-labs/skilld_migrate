 ...
 *   id = "entity_lookup_multiple",
 *   handle_multiples = TRUE
 * )
 *
 * Same plugin as entity_lookup, but with ability to search by several bundles.
 *
 ...
 *     value_key: name
 *     bundle_key: vid
 *     bundles: tags|contacts
 *     entity_type: taxonomy_term
 *     ignore_case: true
 * @endcode
 */
class EntityLookupMultiple extends ProcessPluginBase implements ContainerFactoryPluginInterface {
  ...
  protected function determineLookupProperties($destinationProperty) {
    ...
    if (!empty($this->configuration['bundles'])) {
      $this->lookupBundles = explode('|', $this->configuration['bundles']);
    }
          ...
          case 'entity_reference':
            if (empty($this->lookupBundles)) {
              ...
              elseif (!empty($handlerSettings['auto_create']) && !empty($handlerSettings['auto_create_bundle'])) {
                $this->lookupBundles = reset($handlerSettings['auto_create_bundle']);
              }
            }
    ...
    if (!empty($this->lookupBundleKey) && empty($this->lookupBundles)) {
  ...
  protected function query($value) {
    ...
    if ($this->lookupBundleKey) {
      $query->condition($this->lookupBundleKey, $this->lookupBundles, 'IN');
    }
    ...
    if (!$ignoreCase) {
      ...
      foreach ($results as $k => $identifier) {
        $result_value = $this->entityTypeManager->getStorage($this->lookupEntityType)->load($identifier)->{$this->lookupValueKey}->value;
